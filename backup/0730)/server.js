...
// 1단계 임시 데이터 저장용 캐시
let tempAutobeData = {
    'a-team': { equipmentData: [], timestamp: null },
    'b-team': { equipmentData: [], timestamp: null },
    'c-team': { equipmentData: [], timestamp: null }
};
...
const teamSchedules = {
    'a-team': { 
        step1: { hour: 7, minute: 0, timeFilter: '07:00:00' },  // 1단계: 장비 데이터 추출
        step2: { hour: 7, minute: 5, timeFilter: '07:05:00' },  // 2단계: RPA 데이터 조인
        excludeTimeStart: '07:00:00',
        excludeTimeEnd: '07:05:59'
    },
    'b-team': { 
        step1: { hour: 15, minute: 0, timeFilter: '15:00:00' },
        step2: { hour: 15, minute: 5, timeFilter: '15:05:00' },
        excludeTimeStart: '15:00:00',
        excludeTimeEnd: '15:05:59'
    },
    'c-team': { 
        step1: { hour: 23, minute: 0, timeFilter: '23:00:00' },
        step2: { hour: 23, minute: 5, timeFilter: '23:05:00' },
        excludeTimeStart: '23:00:00',
        excludeTimeEnd: '23:05:59'
    }
};
...
async function getDataFromDB1() {
    ...
    try {
        ...
        const result = await connection.execute(`
            ...
            WHERE i.rn = 1 AND r.rn = 1 AND r.FINALLY NOT IN ('Ok', 'No') AND RPA_STATUS NOT IN('End')
...
// 1단계: 장비 데이터 추출 (RPA_ADMIN.view_mc_eqp_hist)
async function fetchEquipmentDataForTeam(team) {
    ...
    try {
        ...
        console.log(`${team} 1단계 - 장비 데이터 추출 시작...`);
        ...
        const result = await connection.execute(`
            ...
            FROM (
                ...
                WHERE (eqp_id LIKE '%UTS%' OR eqp_id LIKE '%UTC%')
                AND status_seg = 'RUN'
                AND SUBSTR(event_date,1,8) = TO_CHAR(SYSDATE,'YYYYMMDD')
            )
            WHERE rn = 1
            ORDER BY only_date DESC, only_time DESC
        `);

        console.log(`${team} 1단계 완료 - 장비 데이터 ${result.rows ? result.rows.length : 0}건 추출`);

        // 데이터를 JavaScript 객체 배열로 변환
        ...
        console.error(`${team} 1단계 장비 데이터 추출 오류:`, error);
        throw error;
    } finally {
        if (connection) {
            try {
...
// 2단계: RPA 데이터 추출 및 조인 (RPA_ADMIN.rpa)
async function fetchRPADataAndJoinForTeam(team) {
    const schedule = teamSchedules[team];
    if (!schedule) {
        throw new Error(`Unknown team: ${team}`);
    }
    ...
    // 임시 데이터 확인
    const tempData = tempAutobeData[team];
    if (!tempData || !tempData.equipmentData || tempData.equipmentData.length === 0) {
        console.log(`${team} 1단계 데이터가 없어 2단계 실행 중단`);
        return [];
    }
    ...
    try {
        ...
        console.log(`${team} 2단계 - RPA 데이터 추출 및 조인 시작...`);
        ...
        const result = await connection.execute(`
            ...
            FROM (
                ...
                AND NOT (TO_CHAR(start_time,'HH24:MI:SS') BETWEEN :excludeTimeStart AND :excludeTimeEnd)
            )
            WHERE rn = 1
        `, { 
            excludeTimeStart: schedule.excludeTimeStart,
            excludeTimeEnd: schedule.excludeTimeEnd
        });

        console.log(`${team} 2단계 - RPA 데이터 ${result.rows ? result.rows.length : 0}건 추출`);

        // RPA 데이터를 JavaScript 객체 배열로 변환
        const rpaData = result.rows.map(row => ({
            EQPID: row[0],
            RPA_TIME: row[1],
            LOT_ID: row[2],
            DEVICE_ID: row[3],
        ...
        // 장비 데이터와 RPA 데이터 조인
        const joinedData = [];
        tempData.equipmentData.forEach(equipment => {
            const matchingRPA = rpaData.find(rpa => rpa.EQPID === equipment.EQPID);
            if (matchingRPA) {
                joinedData.push({
                    EQPID: equipment.EQPID,
                    RPA_TIME: matchingRPA.RPA_TIME,
                    LOT_ID: matchingRPA.LOT_ID,
                    DEVICE_ID: matchingRPA.DEVICE_ID
                });
            }
        });

        console.log(`${team} 2단계 완료 - 조인된 데이터 ${joinedData.length}건`);
        return joinedData;

    } catch (error) {
        console.error(`${team} 2단계 RPA 데이터 조인 오류:`, error);
        throw error;
    } finally {
        if (connection) {
            try {
...
// 전체 Autobe 데이터 조회 함수 (호환성 유지)
async function fetchAutobeDataForTeam(team) {
    // 2단계 완료 후 최종 데이터 반환
    return await fetchRPADataAndJoinForTeam(team);
}

// 1단계: 장비 데이터 업데이트 함수
async function updateTeamDataStep1(team) {
    try {
        console.log(`=== ${team} 1단계 데이터 업데이트 시작 ===`);
        const equipmentData = await fetchEquipmentDataForTeam(team);
        console.log(`=== ${team} 1단계 데이터 업데이트 완료: ${equipmentData.length}건 ===`);
        
    } catch (error) {
        console.error(`${team} 1단계 데이터 업데이트 실패:`, error);
...
// 2단계: RPA 데이터 조인 및 최종 업데이트 함수
async function updateTeamDataStep2(team) {
    try {
        console.log(`=== ${team} 2단계 데이터 업데이트 시작 ===`);
        const finalData = await fetchRPADataAndJoinForTeam(team);
        ...
        console.log(`=== ${team} 2단계 데이터 업데이트 완룈: ${finalData.length}건 ===`);
        
    } catch (error) {
        console.error(`${team} 2단계 데이터 업데이트 실패:`, error);
        // 실패해도 빈 배열로 설정하여 시스템이 계속 작동하도록 함
        autobeDataCache[team] = {
            data: [],
            lastUpdate: new Date(),
...
// 호환성을 위한 기존 함수 유지 (2단계 완료 후 호출)
async function updateTeamData(team) {
    // 2단계가 완료된 후에만 호출되도록 수정
    await updateTeamDataStep2(team);
}
...
function setupSchedulers() {
    console.log('=== Autobe Check 2단계 스케줄러 초기화 ===');
    ...
    const schedulerInterval = setInterval(() => {
        ...
        // A조 1단계: 매일 07:00
        if (currentTime === '07:00' && lastExecuted['a-team-step1'] !== currentDate) {
            console.log('A조 1단계 스케줄 실행 (07:00)');
            updateTeamDataStep1('a-team').catch(err => console.error('A조 1단계 업데이트 실패:', err));
            lastExecuted['a-team-step1'] = currentDate;
        }
        
        // A조 2단계: 매일 07:05
        if (currentTime === '07:05' && lastExecuted['a-team-step2'] !== currentDate) {
            console.log('A조 2단계 스케줄 실행 (07:05)');
            updateTeamDataStep2('a-team').catch(err => console.error('A조 2단계 업데이트 실패:', err));
            lastExecuted['a-team-step2'] = currentDate;
        }
        
        // B조 1단계: 매일 15:00
        if (currentTime === '15:00' && lastExecuted['b-team-step1'] !== currentDate) {
            console.log('B조 1단계 스케줄 실행 (15:00)');
            updateTeamDataStep1('b-team').catch(err => console.error('B조 1단계 업데이트 실패:', err));
            lastExecuted['b-team-step1'] = currentDate;
        }
        
        // B조 2단계: 매일 15:05
        if (currentTime === '15:05' && lastExecuted['b-team-step2'] !== currentDate) {
            console.log('B조 2단계 스케줄 실행 (15:05)');
            updateTeamDataStep2('b-team').catch(err => console.error('B조 2단계 업데이트 실패:', err));
            lastExecuted['b-team-step2'] = currentDate;
        }
        
        // C조 1단계: 매일 23:00
        if (currentTime === '23:00' && lastExecuted['c-team-step1'] !== currentDate) {
            console.log('C조 1단계 스케줄 실행 (23:00)');
            updateTeamDataStep1('c-team').catch(err => console.error('C조 1단계 업데이트 실패:', err));
            lastExecuted['c-team-step1'] = currentDate;
        }
        
        // C조 2단계: 매일 23:05
        if (currentTime === '23:05' && lastExecuted['c-team-step2'] !== currentDate) {
            console.log('C조 2단계 스케줄 실행 (23:05)');
            updateTeamDataStep2('c-team').catch(err => console.error('C조 2단계 업데이트 실패:', err));
            lastExecuted['c-team-step2'] = currentDate;
        }
        
        // 매일 00:01에 캐시 클리어
        if (currentTime === '06:01' && lastExecuted['cache-clear'] !== currentDate) {
            console.log('일일 캐시 클리어 (06:01)');
            autobeDataCache = {
                'a-team': { data: [], lastUpdate: null },
                'b-team': { data: [], lastUpdate: null },
                'c-team': { data: [], lastUpdate: null }
    ...
    console.log('2단계 스케줄러 설정 완료:');
    console.log('- A조: 07:02(1단계) -> 07:07(2단계)');
    console.log('- B조: 15:02(1단계) -> 15:07(2단계)');
    console.log('- C조: 23:02(1단계) -> 23:07(2단계)');
    console.log('- 시간 제외 범위: A조(07:00-07:05), B조(15:00-15:05), C조(23:00-23:05)');
    console.log('- 캐시 클리어: 매일 00:01');
    console.log('- 체크 간격: 1분');
    
    // 서버 종료 시 타이머 정리
    process.on('SIGINT', () => {
        console.log('서버 종료 중... 2단계 스케줄러 타이머 정리');
        clearInterval(schedulerInterval);
        process.exit(0);
    });
    
    process.on('SIGTERM', () => {
        console.log('서버 종료 중... 2단계 스케줄러 타이먈 정리');
        clearInterval(schedulerInterval);
        process.exit(0);
    });
}
