...
// 조별 시간 설정
const teamSchedules = {
    'a-team': { hour: 7, minute: 2, timeFilter: '07:02:00' },  // 07:05에 실행, 07:00 이전 데이터
    'b-team': { hour: 15, minute: 2, timeFilter: '15:02:00' }, // 15:05에 실행, 15:00 이전 데이터
    'c-team': { hour: 23, minute: 2, timeFilter: '23:02:00' }  // 23:05에 실행, 23:00 이전 데이터
};
...
async function fetchAutobeDataForTeam(team) {
    ...
    // 조별 데이터 필터 기준 시간 고정
    const teamTimeFilters = {
        'a-team': '07:00:00',
        'b-team': '15:00:00',
        'c-team': '23:00:00'
    };
    const filterTime = teamTimeFilters[team];

    let connection;
    try {
        connection = await oracledb.getConnection(dbConfig1);

        console.log(`${team} Autobe 쿼리 실행 시작... (데이터 필터: ${filterTime} 이전, 조회 트리거 시간: ${schedule.timeFilter})`);
        ...
        const result = await connection.execute(`
            ...
            b AS (
                ...
                FROM (
                    ...
                    AND TO_CHAR(start_time,'HH24:MI:SS') < :filterTime
                ) b_sub
                WHERE rn = 1
            )
            SELECT
            ...
            WHERE b.eqp_id IS NOT NULL
            ORDER BY a.only_date DESC, a.only_time DESC
        `, { filterTime });

        console.log(`${team} Autobe 쿼리 실행 완료`);
        ...
        console.log(`${team} 매핑된 데이터 (${data.length}건)`);
        return data;
...
async function updateTeamData(team) {
    try {
        ...
        const data = await fetchAutobeDataForTeam(team);
        ...
        console.log(`=== ${team} 데이터 업데이트 완료: ${data.length}건 ===`);
...
function setupSchedulers() {
    ...
    const schedulerInterval = setInterval(() => {
        ...
        // A조: 매일 07:02
        if (currentTime === '07:02' && lastExecuted['a-team'] !== currentDate) {
            console.log('A조 스케줄 실행 (07:02)');
            updateTeamData('a-team').catch(err => console.error('A조 업데이트 실패:', err));
            lastExecuted['a-team'] = currentDate;
        }
        
        // B조: 매일 15:02
        if (currentTime === '15:02' && lastExecuted['b-team'] !== currentDate) {
            console.log('B조 스케줄 실행 (15:02)');
            updateTeamData('b-team').catch(err => console.error('B조 업데이트 실패:', err));
            lastExecuted['b-team'] = currentDate;
        }
        
        // C조: 매일 23:02
        if (currentTime === '23:02' && lastExecuted['c-team'] !== currentDate) {
            console.log('C조 스케줄 실행 (23:02)');
    ...
    console.log('- A조: 매일 07:05 (07:00 이전 데이터)');
    console.log('- B조: 매일 15:05 (15:00 이전 데이터)');
    console.log('- C조: 매일 23:05 (23:00 이전 데이터)');
